...
namespace Booking.Application.UseCases.Reviews.CreateReview
{
    public sealed class CreateReviewHandler(IReviewRepository _reviewRepo,
        IReviewQueries _reviewQueries, IUnitOfWork _unitOfWork,
        IOutputCacheStore _outputCache, ICacheService _cacheService) : ICommandHandler<CreateReviewCommand, Guid>
    {
        public async Task<Result<Guid>> Handle(CreateReviewCommand request, CancellationToken ct)
        {
            ...
            //invalidates all cached pages for certain target
            if (result.IsSuccess)
            {
                await _outputCache.EvictByTagAsync($"target_{reviewResult.Value.TargetId.ToString().ToLowerInvariant()}", ct);
            }

            //invalidates room/listing distributed cache
            if (reviewResult.Value!.TargetType == ReviewsTargetType.Room)
            {
                await _cacheService.RemoveAsync($"room:{reviewResult.Value.TargetId}", ct);
            }

            if (reviewResult.Value!.TargetType == ReviewsTargetType.Listing)
            {
                await _cacheService.RemoveAsync($"listing:{reviewResult.Value.TargetId}", ct);
            }
